...
const InlineCalculator: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[340px] bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 shadow-[0_30px_90px_rgba(0,0,0,0.6)] rounded-[2.5rem] p-7 animate-in fade-in zoom-in duration-300">
        ...
        <div className="bg-slate-100 dark:bg-slate-950 p-6 rounded-[1.5rem] mb-6 text-right border border-slate-200 dark:border-slate-800 shadow-inner">
...
const App: React.FC = () => {
    ...
    const loadFromSupabase = async () => {
      try {
        ...
        if (data && data.length > 0) {
          const products: ProductData[] = data.map((row: any) => ({
            ...row.data,
            id: row.id,
            name: row.name,
            lastModified: row.last_modified,
          }));
          const lastId = localStorage.getItem(DB_KEY + '_lastId') || products[0].id;
          const newDb: AppDatabase = { version: '1.0', products, lastSelectedProductId: lastId };
          setDb(newDb);
          localStorage.setItem(DB_KEY, JSON.stringify(newDb));
          setSyncStatus('synced');
        } else {
          // Nenhum dado na nuvem — faz upload do localStorage
          setSyncStatus('idle');
        }
      } catch (err) {
        console.warn('Supabase offline, usando dados locais:', err);
        setSyncStatus('error');
      }
    };
    loadFromSupabase();
  ...
  // Salva produto no Supabase em background
  const saveProductToSupabase = useCallback(async (product: ProductData) => {
    try {
      const { id, name, lastModified, ...rest } = product;
      await supabase.from('calc_products').upsert({
        id,
        name,
  ...
  const deleteProductFromSupabase = useCallback(async (id: string) => {
    try {
      await supabase.from('calc_products').delete().eq('id', id);
    } catch (err) {
  ...
  const persistData = useCallback(() => {
    ...
    // Salva o produto atual no Supabase em background
    const current = db.products.find(p => p.id === db.lastSelectedProductId);
    if (current) saveProductToSupabase(current);
    setTimeout(() => {
      setSaveStatus('saved');
      setTimeout(() => setSaveStatus('idle'), 1500);
    }, 200);
  }, [db, saveProductToSupabase]);
  ...
  const handleExportBackup = () => {
    ...
    link.href = url;
    link.download = `backup_preco_pro_${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  const syncAllToSupabase = async () => {
    ...
    try {
      for (const product of db.products) {
        const { id, name, lastModified, ...rest } = product;
        await supabase.from('calc_products').upsert({
          id, name, last_modified: lastModified, data: rest,
        }, { onConflict: 'id' });
      }
      setSyncStatus('synced');
      alert('✅ Todos os produtos sincronizados com a nuvem!');
    } catch (err) {
      setSyncStatus('error');
  ...
  const restoreFromCloud = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const products: ProductData[] = data.map((row: any) => ({
          ...row.data, id: row.id, name: row.name, lastModified: row.last_modified,
        }));
        const newDb: AppDatabase = { version: '1.0', products, lastSelectedProductId: products[0].id };
        setDb(newDb);
        localStorage.setItem(DB_KEY, JSON.stringify(newDb));
        setShowLibrary(false);
        setSyncStatus('synced');
  ...
  const inputBase = "w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-2.5 text-[12px] font-bold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-slate-800 dark:text-slate-100";

  return (
    <div className="min-h-screen pb-10 bg-slate-200 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors font-sans overflow-x-hidden">

      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 px-3 sm:px-6 h-14 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <button onClick={() => exportToPDF(currentProduct.insumos || [], summary, currentProduct.name, currentProduct.terceirizados || [])} title="Exportar para PDF" aria-label="Exportar para PDF" className="bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider shadow-md active:scale-95 transition-transform"><Download className="w-3.5 h-3.5" /> <span className="hidden sm:inline">PDF</span></button>
        </div>
      </header>
      ...
      <main className="max-w-[1440px] mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <Section title="1. Detalhamento de Insumos" icon={<Package className="text-emerald-500 w-5 h-5" />} expanded={expandedSection === 'insumos'} onToggle={() => toggleSection('insumos')}>
            ...
            <div className="md:hidden space-y-4 mb-4 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {currentProduct.insumos.map((insumo) => (
                <div key={insumo.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm relative group">
                  <div className="mb-4">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button onClick={() => updateCurrentProduct({ insumos: currentProduct.insumos.filter(i => i.id !== insumo.id) })} className="flex items-center gap-2 px-3 py-2 text-[10px] font-bold text-red-500 bg-red-50 dark:bg-red-900/10 rounded-lg hover:bg-red-100 transition-colors">
            ...
            <div className="hidden md:block overflow-x-auto custom-scrollbar -mx-2 px-2">
              <div className="min-w-[780px] pb-4">
                ...
                <div className="space-y-2 p-1">
                  {currentProduct.insumos.map((insumo) => (
                    <div key={insumo.id} className="grid grid-cols-[2fr_1fr_1fr_1.5fr_1.2fr_0.5fr] gap-3 p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl items-center hover:border-blue-400 transition-all shadow-sm">
                      <input value={insumo.nome} title="Nome do Material" onChange={(e) => updateCurrentProduct({ insumos: currentProduct.insumos.map(i => i.id === insumo.id ? { ...i, nome: e.target.value } : i) })} className={`${inputBase} !bg-transparent truncate`} />
                      ...
                      <button onClick={() => updateCurrentProduct({ insumos: currentProduct.insumos.filter(i => i.id !== insumo.id) })} title="Excluir Material" aria-label="Excluir Material" className="text-slate-300 hover:text-red-500 mx-auto"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <Section title="2. Mão de Obra e Serviços" icon={<Users className="text-orange-500 w-5 h-5" />} expanded={expandedSection === 'terceirizados'} onToggle={() => toggleSection('terceirizados')}>
            ...
            <div className="md:hidden space-y-4 mb-4 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {currentProduct.terceirizados.map((t) => (
                <div key={t.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm relative group">
                  <div className="mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                {currentProduct.custosFixos.map(cf => (
                  <div key={cf.id} className="flex gap-2 items-center bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-200 dark:border-slate-800">
                    <input value={cf.nome} title="Nome do Custo Fixo" onChange={(e) => updateCurrentProduct({ custosFixos: currentProduct.custosFixos.map(i => i.id === cf.id ? { ...i, nome: e.target.value } : i) })} className="flex-1 bg-transparent border-none text-[12px] font-bold outline-none" />
                    <input type="text" value={getDisplayValue(cf.valor, cf.id, 'f')} title="Valor do Custo Fixo" onBlur={() => setEditingValue(null)} onChange={(e) => handleNumericChange(cf.id, 'f', e.target.value, (v) => updateCurrentProduct({ custosFixos: currentProduct.custosFixos.map(i => i.id === cf.id ? { ...i, valor: v } : i) }))} className="w-24 bg-white dark:bg-slate-800 border rounded-md px-2 py-1 text-[12px] text-right font-mono focus:ring-2 focus:ring-blue-500 outline-none" />
                    <button onClick={() => updateCurrentProduct({ custosFixos: currentProduct.custosFixos.filter(i => i.id !== cf.id) })} title="Excluir Custo Fixo" aria-label="Excluir Custo Fixo" className="text-slate-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                ...
                {currentProduct.custosIndiretos.map(ci => (
                  <div key={ci.id} className="flex gap-2 items-center bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-200 dark:border-slate-800">
                    <input value={ci.nome} title="Nome do Custo Variável" onChange={(e) => updateCurrentProduct({ custosIndiretos: currentProduct.custosIndiretos.map(i => i.id === ci.id ? { ...i, nome: e.target.value } : i) })} className="flex-1 bg-transparent border-none text-[12px] font-bold outline-none" />
                    <input type="text" value={getDisplayValue(ci.valor, ci.id, 'i')} title="Valor do Custo Variável" onBlur={() => setEditingValue(null)} onChange={(e) => handleNumericChange(ci.id, 'i', e.target.value, (v) => updateCurrentProduct({ custosIndiretos: currentProduct.custosIndiretos.map(i => i.id === ci.id ? { ...i, valor: v } : i) }))} className="w-24 bg-white dark:bg-slate-800 border rounded-md px-2 py-1 text-[12px] text-right font-mono focus:ring-2 focus:ring-blue-500 outline-none" />
                    <button onClick={() => updateCurrentProduct({ custosIndiretos: currentProduct.custosIndiretos.filter(i => i.id !== ci.id) })} title="Excluir Custo Variável" aria-label="Excluir Custo Variável" className="text-slate-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
            ...
            <Section title="5. Metas e Taxas" icon={<Target className="text-indigo-500 w-5 h-5" />} expanded={expandedSection === 'markup'} onToggle={() => toggleSection('markup')}>
              <div className="grid grid-cols-3 gap-2">
                <div className="text-center">
                  ...
                  <input type="text" value={getDisplayValue(currentProduct.markup?.impostos || 0, 'm', 'i')} title="Impostos %" onBlur={() => setEditingValue(null)} onChange={(e) => handleNumericChange('m', 'i', e.target.value, (v) => updateCurrentProduct({ markup: { ...currentProduct.markup, impostos: v } }))} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 rounded-lg py-2.5 text-[14px] font-black text-center font-mono focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div className="text-center">
                  <label className="text-[9px] font-black text-blue-500 uppercase block mb-1">Meta %</label>
                  <input type="text" value={getDisplayValue(currentProduct.markup?.margemLucro || 0, 'm', 'l')} title="Margem de Lucro %" onBlur={() => setEditingValue(null)} onChange={(e) => handleNumericChange('m', 'l', e.target.value, (v) => updateCurrentProduct({ markup: { ...currentProduct.markup, margemLucro: v } }))} className="w-full bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg py-2.5 text-[14px] font-black text-center text-blue-700 dark:text-blue-400 font-mono focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div className="text-center">
                  <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Perda %</label>
                  <input type="text" value={getDisplayValue(currentProduct.markup?.perdas || 0, 'm', 'p')} title="Perdas %" onBlur={() => setEditingValue(null)} onChange={(e) => handleNumericChange('m', 'p', e.target.value, (v) => updateCurrentProduct({ markup: { ...currentProduct.markup, perdas: v } }))} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 rounded-lg py-2.5 text-[14px] font-black text-center font-mono focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
              </div>
            </Section>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="relative mb-8 bg-slate-800/50 p-6 rounded-3xl border border-slate-700/50 backdrop-blur-sm">
              <span className="absolute left-6 top-1/2 -translate-y-1/2 text-2xl font-black text-slate-500">R$</span>
            ...
            <div className="space-y-4 pt-6 border-t border-slate-800">
              ...
              <div className={`p-6 rounded-[2rem] border-2 transition-all ${summary.margemReal >= (currentProduct.markup?.margemLucro || 0) ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-orange-500/10 border-orange-500/30'}`}>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex justify-between text-xs font-bold"><span>Materiais</span><span className="font-mono">{formatCurrency(summary.custoMaterial)}</span></div>
              <div className="flex justify-between text-xs font-bold"><span>Serviços</span><span className="font-mono">{formatCurrency(summary.custoTerceirizados)}</span></div>
              <div className="flex justify-between text-xs font-bold"><span>Operacional</span><span className="font-mono">{formatCurrency(summary.custoFixoPorUnidade)}</span></div>
              <div className="flex justify-between text-xs font-bold text-red-500"><span>Perdas</span><span className="font-mono">{formatCurrency(summary.valorPerdaUnitario)}</span></div>
              <div className="pt-5 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <span className="font-black text-[11px] uppercase text-slate-500">Total de Fábrica</span>
                <span className="text-2xl font-black font-mono tracking-tighter">{formatCurrency(summary.custoProducaoUnitario)}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {showLibrary && (
        <div className="fixed inset-0 z-[1500] flex items-center justify-start">
          ...
          <div className="relative w-80 h-full bg-white dark:bg-slate-950 p-8 shadow-2xl animate-in slide-in-from-left duration-300 border-r border-slate-200 dark:border-slate-800 flex flex-col">
            ...
            <button onClick={() => { const p = DEFAULT_PRODUCT(Math.random().toString(36)); setDb(prev => ({ ...prev, products: [...prev.products, p], lastSelectedProductId: p.id })); setShowLibrary(false); }} className="w-full py-4 mb-8 bg-blue-600 text-white rounded-[1.2rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-blue-700 shadow-xl active:scale-95 transition-all"><Plus className="w-4 h-4" /> NOVO PRODUTO</button>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
              {db.products.map(p => (
                <div key={p.id} className="group flex gap-2 animate-in fade-in duration-500">
                  ...
                  {db.products.length > 1 && <button onClick={() => { const id = p.id; setDb(prev => { const next = prev.products.filter(item => item.id !== id); deleteProductFromSupabase(id); return { ...prev, products: next, lastSelectedProductId: next[0].id }; }); }} title="Excluir Produto" aria-label="Excluir Produto" className="p-2 text-slate-300 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>}
                </div>
              ))}
            </div>

            <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
              <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Sincronização e Segurança</h3>

              {/* Status Supabase */}
              <div className={`flex items-center gap-2 mb-4 px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider ${syncStatus === 'synced' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600' :
                syncStatus === 'syncing' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-500' :
                ...
                {syncStatus === 'synced' ? 'Sincronizado ✓' :
                  syncStatus === 'syncing' ? 'Sincronizando...' :
                    syncStatus === 'error' ? 'Erro de conexão' : 'Aguardando sync'}
              </div>

              <div className="grid grid-cols-2 gap-2 mb-3">
                <button onClick={handleExportBackup} className="flex flex-col items-center justify-center p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all group">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center justify-center p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition-all group">
                  <Upload className="w-4 h-4 text-emerald-500 mb-1" />
                  <span className="text-[8px] font-black uppercase text-slate-500 group-hover:text-emerald-600">Importar</span>
                </button>
              ...
              <button onClick={syncAllToSupabase} disabled={syncStatus === 'syncing'} className="w-full py-3 bg-slate-900 dark:bg-slate-800 text-white rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-slate-800 transition-all disabled:opacity-50">
                <Cloud className="w-4 h-4 text-blue-400" /> Enviar para Nuvem
              </button>
              <button onClick={restoreFromCloud} disabled={syncStatus === 'syncing'} className="w-full mt-2 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:text-blue-500 flex items-center justify-center gap-1 disabled:opacity-50">
                <RefreshCw className="w-3 h-3" /> Restaurar da Nuvem
              </button>
            </div>
          </div>
        </div>
...
const Section: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode; expanded: boolean; onToggle: () => void; }> = ({ title, icon, children, expanded, onToggle }) => (
  <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm transition-all duration-300 overflow-hidden">
    <button onClick={onToggle} className={`w-full px-8 py-6 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors ${expanded ? 'bg-slate-50/50 dark:bg-slate-800/20' : ''}`}>
      <div className="flex items-center gap-5"><div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-[1.2rem] shadow-sm">{icon}</div><span className="font-black text-[13px] uppercase tracking-[0.2em] text-slate-700 dark:text-slate-200">{title}</span></div>
      <ChevronDown className={`w-6 h-6 text-slate-300 transition-all duration-300 ${expanded ? 'rotate-180 text-blue-600' : ''}`} />
    </button>
    <div className={`transition-all duration-500 ease-in-out ${expanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'} overflow-visible`}>
      <div className="px-8 pb-10 pt-4 border-t border-slate-100 dark:border-slate-800/50">{children}</div>
    </div>
  </div>
);
